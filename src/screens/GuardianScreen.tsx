...
const GuardianScreen = () => {
    ...
    // 2. Connect & Subscribe to Telemetry
    const handleConnect = () => {
        if (!targetUid.trim()) {
        ...
        // Subscribe to 'active_alerts' (Where SOS data lives)
        const unsubscribe = onSnapshot(doc(db, 'active_alerts', targetUid.trim()), (docSnap) => {
            if (docSnap.exists()) {
                const data = docSnap.data();
                setChildData(data);
                setIsConnected(true);
            } else {
                // Only alert if we were expecting a connection, but not constantly
                if (!isConnected) {
    ...
    const getBatteryColor = (level: number) => {
        if (level < 0) return '#9ca3af'; // Unknown
        if (level <= 0.2) return '#ef4444'; // Red
        return '#22c55e'; // Green
    };
    ...
    const speed = childData?.speed ? (childData.speed * 3.6).toFixed(1) : '0.0'; // m/s to km/h
            ...
            <View style={styles.hudContainer}>
                <View style={styles.hudCard}>
                    ...
                    <View style={styles.hudHeader}>
                        ...
                        <Text style={styles.lastUpdated}>{formatTime(childData?.lastUpdated)}</Text>
                    </View>
